...
from RV.RV_Net_Parts import RV_2foldloss
from RV.RV_Dataset_Preperation_Torch import BasicDataset
...
def train_net(net,
    ...
    image_max = np.max(dataset)

    n_val = int(len(dataset) * val_percent)
    n_train = len(dataset) - n_val
    # train, val = random_split(dataset, [n_train, n_val])
    # data = dataset.unsqueeze(1)
    ...
    if net.n_classes > 1:
        ...
        criterion = RV_2foldloss(alpha=0.5, image_max=image_max)
    else:
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                targets = targets.to(device=device, dtype=target_type)

                rec_imgs, classified_pred = net(imgs)
                loss, reconstroction_loss_temp, classification_loss_temp = criterion(reconstroction_predicted=rec_imgs, reconstroction_target=targets, classification_predicted=classified_pred, classification_target=label)
                # rec_losses_temp.append(reconstroction_loss_temp.cpu().detach().numpy())
                # class_losses_temp.append(classification_loss_temp.cpu().detach().numpy())
                epoch_loss += loss.item()
                # print("reconstroction_loss_temp", reconstroction_loss_temp)
                # print("classification_loss_temp", classification_loss_temp)
        ...
        torch.save(net.state_dict(), 'path to save model_epoch{}.pth'.format(epoch))
                ...
                #     if net.n_classes == 1:
                #         writer.add_images('target', targets, global_step)
